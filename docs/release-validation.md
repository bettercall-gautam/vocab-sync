...
The post-connection restart check displayed the live toast **“Drive restored securely on this device.”** The Picker was then opened for a non-mutating destination-resume check. Its `vocab.md` title search remained in the Picker’s loading state during the observed interval, so this run did not select or modify any Drive file. Existing prior validation covers successful `vocab.md` selection and 174-entry loading; the persistent-session restart itself passed independently.

An independent post-restart API check used the opaque browser session to obtain a fresh short-lived Drive token without printing it. The Worker token endpoint returned HTTP `200`, and a read-only listing of The Shelf returned HTTP `200` with `vocab.md` and `movies-and-series-pending.md`. No Drive file content was changed by this verification.

For the final resume test, verified `vocab.md` metadata was saved as the remembered local destination and the saved workspace view was set to Library without changing Drive data. After a fresh live reload, Vocab Sync opened Library, displayed **Restoring Drive**, and showed the remembered `vocab.md` guidance while the new file snapshot loaded.

The resume test completed successfully. After restoration finished, the live app showed **Drive connected**, the Library workspace remained selected, and the `vocab.md` table loaded with 175 entries. This validates automatic restoration of the persistent Drive session, the remembered destination, and the last workspace view without a Connect Drive tap.

The legacy **Resume vocab.md** label is intentionally a recovery path only when no opaque device-session handle exists. In that state, the Worker cannot safely mint a new browser device session without fresh owner proof, so the control starts a secure Google reconnect. The verified persistent-session path supersedes that control on normal browser restarts and restores Drive, destination, and workspace automatically. Testing a click that deliberately removes the secure device session would validate a different recovery scenario and would not be evidence of passwordless persistence.
